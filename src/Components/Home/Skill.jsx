            ...
            {item.skills?.map((skill, skillIndex) => {
              return (
                <motion.div
                  ...
                  className="group/skill flex cursor-default items-center gap-3 rounded-xl border border-white/[0.06] bg-[#131620] px-3 py-2.5 shadow-2xl transition-all duration-300 hover:-translate-y-0.5 hover:border-[#f94144]/50 hover:bg-[#DC2F02]/[0.08] hover:shadow-[0_5px_20px_rgba(220,47,2,0.10)]"
                >
                  {/* Dynamic Icon */}
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white/[0.04] transition-all duration-300 group-hover/skill:bg-[#DC2F02]/10">
                    {skill.icon ? (
                      <img
                        src={skill.icon}
                        alt={skill.name}
                        className="h-4 w-4 object-contain transition-all duration-300 group-hover/skill:scale-110"
                      />
                    ) : (
...
const Skill = () => {
    ...
    const getSkills = async () => {
      try {
        const [
          ...
          fetch(`${baseUrl}/api/frontendskill/get`),
          fetch(`${baseUrl}/api/backendskill/get`),
          fetch(`${baseUrl}/api/databaseskill/get`),
          fetch(`${baseUrl}/api/toolskill/get`),
        ]);
        ...
        setDatabaseSkills(databaseData);
        setTools(toolsData);
  ...
  console.log("databaseSkills:", databaseSkills);
                    ...
                    >
                      ...
                      {tool.icon ? (
                        <img
                          src={tool.icon}
                          alt={tool.name}
                          className="h-4 w-4 shrink-0 object-contain transition-all duration-300 group-hover/tool:scale-110"
                        />
                      ) : (
                        <span className="text-xs text-gray-500">
                          ?
